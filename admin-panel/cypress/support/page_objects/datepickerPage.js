function selectMonthAndYear(day) {
  let date = new Date();
...
export class DatepickerPage {
  selectCommonDatepcikerDateFromToday(dayFromToday) {
    cy.contains("nb-card", "Common Datepicker")
      .find("input")
      .then((input) => {
        cy.wrap(input).click();

        const dateToAssert = selectMonthAndYear(dayFromToday);
  ...
  selectDatepickerWithRangeFromToday(firstDay, secondDay) {
    cy.contains("nb-card", "Datepicker With Range")
      ...
      .then((input) => {
        ...
        const firstDateToAssert = selectMonthAndYear(firstDay);
        const secondDateToAssert = selectMonthAndYear(secondDay);
